...
RUN curl --silent "https://api.github.com/repos/binance-chain/bsc/releases/tags/v1.1.8" | jq -c '.assets[] | select( .browser_download_url | contains("mainnet.zip") or contains("geth_linux")) | .browser_download_url' | xargs -n1 curl -LOJ && \
    unzip mainnet.zip -d / && \
